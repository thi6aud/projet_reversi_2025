...
class Board:
    """Initialisation du plateau du début de partie avec les 4 pions centraux"""
    ...
    def get_valid_moves(self, player):
        """Vérifie les mouvement possibles et proposent ceux qui permettent de capturer au moins un jeton adverse."""
        valid_moves = []
    ...
    def apply_move(self, col, row, player):
        """Prend en compte le choix du joueur, vérifie s’il est parmi les coups valides place le jeton et retourne les jetons capturés"""
    ...
    def is_terminal(self):
        """vérifie s’il reste des coups valides pour les 2 joueurs, renvoie True si aucun coups possibles donc fin de partie"""
        return not self.get_valid_moves(BLUE) and not self.get_valid_moves(PINK)

    def score(self, player):
        """calcul le score du joueur = la différence entre les jetons du joueur moins ceux de l’adversaire"""
        blue_count, pink_count = self.count_discs()
